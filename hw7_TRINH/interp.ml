...
and
  ...
  | Variant(tag,args) ->
    let rec f arg = (
      match arg with
      | [] -> []
      | e::es -> (eval_expr en e):: (f es)) in
    TaggedVariantVal (tag, (f args))
  | Case(cond,branches) -> 
      let rec f n l e =
      (match n with
        |[] -> e
        | x::xs -> f xs (List.tl l) (extend_env e x (List.hd l))) in
      let condition = eval_expr en cond in
      let rec f2 x =
      (match x with
        | [] -> failwith "No branches match the condition"
        | x::xs -> (
          match x with
          | Branch(a,b,c) -> (
            match condition with
            | TaggedVariantVal(tag,args) -> if tag=a then eval_expr (f b args en) c else f2 xs
            | _ -> f2 xs))) in f2 branches
  | Debug ->
